function [vector] = obtener_probabilidades_esp(archivo)
    c_especiales = 0;   % Contador de caracteres especiales
    ...
    for r = 123:255
        ...
        elseif(r == 209 || r == 241)
            otro = char(r); 
            total_n = total_n + length(strfind(letrasUpper,otro)); % Buscando el numero de ocurrencias(ñ)
        else   
            simbolo = char(r);                              % Convirtiendo de código ASCII a símbolo.
            total = length(strfind(letrasUpper,simbolo));   % Buscando el numero de ocurrencias
            c_especiales = c_especiales + total;            % Aumentando el contador de especiales
    ...
    % Probabilidad de la Ñ-ñ
    proba_n = total_n / (total_letras - c_especiales); %Calculo de la probabilidad omiento caracretes especiales
